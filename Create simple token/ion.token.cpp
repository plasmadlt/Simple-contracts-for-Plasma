...
void token::issue( name to, asset quantity, string memo )
{
   ...
   // search for a token among existing tokens, a common table, if not found, an error and return
    stats statstable( _self, sym.code().raw() );
    auto existing = statstable.find( sym.code().raw() );
 ...
     // increase the supply parameter - the total number of coins on the platform
  ...
  // call the function of adding balance for the user, for the number of coins - specified in the quantity parameter
    add_balance( st.issuer, quantity, st.issuer );

    if( to != st.issuer ) {
  // call the function of sending the token to the user
...
void token::retire( asset quantity, string memo )
{
  ...
  // check for the validity of the number of coins for burning - input parameter
    check( quantity.is_valid(), "invalid quantity" );
...
void token::sub_balance( name owner, asset value ) {
   ...
   // generate account key for search
   ...
   // change the balance - decrease by the input parameter value
...
void token::add_balance( name owner, asset value, name ram_payer )
{
   ...
   } else { // otherwise - modification of the current record to the value of the input parameter value
...
// create an account for the user on which it can contain a token of a certain format
void token::open( name owner, const symbol& symbol, name ram_payer )
{
   ...
   // search for the character code in the table and check for correctness
...
void token::transfer_(name from, name to, const asset& quantity, const string& memo)
{
   ...
   // check the existence of an account inside the blockchain
   check( is_account( to ), "to account does not exist");

   // formation and search for the symbolic code of the coin
   ...
   // payer for creating / forming a record in the table
   auto payer = has_auth( to ) ? to : from;

   // call a service function - decrease user balance
   sub_balance( from, quantity );

  // call a service function - increase user balance
